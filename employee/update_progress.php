...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validate data
    if (empty($inquiry_id) || !is_numeric($progress) || empty($status)) {
    ...
    $update_query = "UPDATE service_inquiries SET progress = ?, status = ? WHERE id = ?";
    $stmt = $conn->prepare($update_query);
    $stmt->bind_param("isi", $progress, $status, $inquiry_id);
    
    if ($stmt->execute()) {
        header("Location: employee_inquiries.php?success=progress_updated");
    } else {
        header("Location: employee_inquiries.php?error=failed");
    }
